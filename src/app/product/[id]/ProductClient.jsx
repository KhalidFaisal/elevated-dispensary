  ...
  return (
    <div className="pt-24 pb-16 min-h-screen">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <Link href="/menu" className="inline-flex items-center gap-2 text-elevated-emerald hover:text-elevated-emerald-light transition-colors mb-8 font-medium">
        ...
        <div className="glass-card overflow-hidden">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5">
            {/* Image Section */}
            <div className="lg:col-span-2 flex flex-col bg-elevated-dark border-r border-elevated-border">
              <div className="relative aspect-square md:aspect-auto flex-1">
                {mainImage ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center min-h-[400px]">
                    <svg className="w-24 h-24 text-elevated-border" viewBox="0 0 24 24" fill="currentColor">
                ...
                {product.featured && (
                  <div className="absolute top-4 left-4 bg-elevated-gold text-elevated-black text-sm font-bold px-3 py-1.5 rounded-full flex items-center gap-1 shadow-lg">
                    <svg className="w-4 h-4 text-elevated-black" fill="currentColor" viewBox="0 0 24 24"><path d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z"/></svg>
                    Featured
                  </div>
                )}
              </div>
              ...
              {/* Thumbnail Gallery */}
              {images.length > 1 && (
                <div className="grid grid-cols-4 gap-2 p-4 border-t border-elevated-border bg-elevated-card">
                  {images.map((img, i) => (
                    <button
                      key={i}
                      onClick={() => setMainImage(img)}
                      className={`relative aspect-square rounded-lg overflow-hidden border-2 transition-all ${
                        mainImage === img ? 'border-elevated-emerald opacity-100' : 'border-transparent opacity-60 hover:opacity-100 hover:border-elevated-border'
                      }`}
                    >
            ...
            <div className="lg:col-span-3 p-8 md:p-12 flex flex-col">
              ...
              {product.weight && (
                <div className="flex flex-wrap items-center gap-6 mb-8 text-lg border-b border-elevated-border pb-8">
                  <span className="text-elevated-muted font-medium flex items-center gap-2">
              ...
              {product.description && (
                <div className="mb-10">
                  ...
                  <p className="text-elevated-muted text-lg leading-relaxed">
                    {product.description}
                  </p>
                </div>
              )}
              ...
              <div className="mt-auto">
                ...
                <div className="flex flex-col sm:flex-row items-end sm:items-center justify-between gap-6">
                  <div className="w-full sm:w-auto">
                    <span className="block text-elevated-muted mb-1 text-sm">Price</span>
                    {product.calculatedDiscountPrice && product.calculatedDiscountPrice < product.price ? (
                      <div className="flex flex-col">
                        <span className="text-lg text-elevated-muted line-through decoration-red-500/80">
                  ...
                  <div className="flex items-center gap-4 w-full sm:w-auto">
                    {/* Quantity Selector */}
                    {product.stock > 0 && (
                      <div className="flex items-center bg-elevated-dark rounded-xl border border-elevated-border overflow-hidden">
                        <button 
                          onClick={() => setQuantity(Math.max(1, quantity - 1))}
                          className="px-4 py-3 text-elevated-muted hover:text-white hover:bg-white/5 transition-colors"
                        >
                          -
                        </button>
